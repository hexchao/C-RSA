...
def randk_w(wk,w0):
    w_size = w0.size()
    w1 = wk-w0
    w2 = (10000)*wk-w0#Large-number Attack
    #w2 = (-1)*wk-w0#Sign-flipping Attack
    #w2 = float(np.random.randn(1))-w0#Gaussian Attack
    w1 = w1.view([-1])
    w2 = w2.view([-1])
    ratio = Q_ratio
    paralength = len(w1)
...
def randk_b(wk,w0):
    w1 = wk-w0
    w2 = (10000)*wk-w0#Byzantine Attack
    #w2 = (-1)*wk-w0#Sign-flipping Attack
    #w2 = float(np.random.randn(1))-w0#Gaussian Attack
    ratio = Q_ratio
    paralength = len(w1)
...
for k in range(Node):
    ...
    sign_fc2_w_N.append([])
    sign_fc3_w_N.append([])
    sign_conv1_w_N.append([])
    sign_conv2_w_N.append([])
    sign_fc1_b_N.append([])
    sign_fc2_b_N.append([])
    sign_fc3_b_N.append([])
    sign_conv1_b_N.append([])
    sign_conv2_b_N.append([])
    sign_fc1_w_B.append([])
    sign_fc2_w_B.append([])
    sign_fc3_w_B.append([])
    sign_conv1_w_B.append([])
    sign_conv2_w_B.append([])
    sign_fc1_b_B.append([])
    sign_fc2_b_B.append([])
    sign_fc3_b_B.append([])
    sign_conv1_b_B.append([])
    sign_conv2_b_B.append([])
...
for epoch in range(iteration):
    ...
    for i in range(steps):
        ...
        for k in range(Node):
            sign_fc1_w_N[k],sign_fc1_w_B[k] = randk_w(net[k].fc1.weight.data.clone(),net0.fc1.weight.data.clone())
            sign_fc2_w_N[k],sign_fc2_w_B[k] = randk_w(net[k].fc2.weight.data.clone(),net0.fc2.weight.data.clone())
            sign_fc3_w_N[k],sign_fc3_w_B[k] = randk_w(net[k].fc3.weight.data.clone(),net0.fc3.weight.data.clone())
            sign_conv1_w_N[k],sign_conv1_w_B[k] =  randk_w(net[k].conv1.weight.data.clone(),net0.conv1.weight.data.clone())
            sign_conv2_w_N[k],sign_conv2_w_B[k] = randk_w(net[k].conv2.weight.data.clone(),net0.conv2.weight.data.clone())
            sign_fc1_b_N[k],sign_fc1_b_B[k] = randk_b(net[k].fc1.bias.data.clone(),net0.fc1.bias.data.clone())
            sign_fc2_b_N[k],sign_fc2_b_B[k] = randk_b(net[k].fc2.bias.data.clone(),net0.fc2.bias.data.clone())
            sign_fc3_b_N[k],sign_fc3_b_B[k] = randk_b(net[k].fc3.bias.data.clone(),net0.fc3.bias.data.clone())
            sign_conv1_b_N[k],sign_conv1_b_B[k] =  randk_b(net[k].conv1.bias.data.clone(),net0.conv1.bias.data.clone())
            sign_conv2_b_N[k],sign_conv2_b_B[k] =  randk_b(net[k].conv2.bias.data.clone(),net0.conv2.bias.data.clone())
        for k in range(Node):
            net_fc1_weight_grad.append(penal * sign_fc1_w_N[k])
            net_fc2_weight_grad.append(penal * sign_fc2_w_N[k])
            net_fc3_weight_grad.append(penal * sign_fc3_w_N[k])
            net_conv1_weight_grad.append(penal * sign_conv1_w_N[k])
            net_conv2_weight_grad.append(penal * sign_conv2_w_N[k])
            net_fc1_bias_grad.append(penal * sign_fc1_b_N[k])
            net_fc2_bias_grad.append(penal * sign_fc2_b_N[k])
            net_fc3_bias_grad.append(penal * sign_fc3_b_N[k])
            net_conv1_bias_grad.append(penal *sign_conv1_b_N[k])
            net_conv2_bias_grad.append(penal *sign_conv2_b_N[k])
            #上面这一步，k个节点不加攻击，收到的x0是清白的。下面这一步，x0收到的xk是被攻击了的
        net0_fc1_weight_grad = regu * net0.fc1.weight.data.clone() + penal * torch.stack([(-1)*sign_fc1_w_B[k] if k<B else (-1)*sign_fc1_w_N[k] for k in range(10)],2).sum(axis=2)
        net0_fc2_weight_grad = regu * net0.fc2.weight.data.clone() + penal * torch.stack([(-1)*sign_fc2_w_B[k] if k<B else (-1)*sign_fc2_w_N[k] for k in range(10)],2).sum(axis=2)
        net0_fc3_weight_grad = regu * net0.fc3.weight.data.clone() + penal * torch.stack([(-1)*sign_fc3_w_B[k] if k<B else (-1)*sign_fc3_w_N[k] for k in range(10)],2).sum(axis=2)
        net0_conv1_weight_grad = regu * net0.conv1.weight.data.clone() + penal * torch.stack([(-1)*sign_conv1_w_B[k] if k<B else (-1)*sign_conv1_w_N[k] for k in range(10)],4).sum(axis=4)
        net0_conv2_weight_grad = regu * net0.conv2.weight.data.clone() + penal * torch.stack([(-1)*sign_conv2_w_B[k] if k<B else (-1)*sign_conv2_w_N[k] for k in range(10)],4).sum(axis=4)
        net0_fc1_bias_grad = regu * net0.fc1.bias.data.clone() + penal * torch.stack([(-1)*sign_fc1_b_B[k] if k<B else (-1)*sign_fc1_b_N[k] for k in range(10)],1).sum(axis=1)
        net0_fc2_bias_grad = regu * net0.fc2.bias.data.clone() + penal * torch.stack([(-1)*sign_fc2_b_B[k] if k<B else (-1)*sign_fc2_b_N[k] for k in range(10)],1).sum(axis=1)
        net0_fc3_bias_grad = regu * net0.fc3.bias.data.clone() + penal * torch.stack([(-1)*sign_fc3_b_B[k] if k<B else (-1)*sign_fc3_b_N[k] for k in range(10)],1).sum(axis=1)
        net0_conv1_bias_grad = regu * net0.conv1.bias.data.clone() + penal * torch.stack([(-1)*sign_conv1_b_B[k] if k<B else (-1)*sign_conv1_b_N[k] for k in range(10)],1).sum(axis=1)
        net0_conv2_bias_grad = regu * net0.conv2.bias.data.clone() + penal * torch.stack([(-1)*sign_conv2_b_B[k] if k<B else (-1)*sign_conv2_b_N[k] for k in range(10)],1).sum(axis=1)
        #再更新（epoch+1）
        for k in range(Node):
